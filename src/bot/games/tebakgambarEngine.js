...
const SESSION_TTL_MS = Number(process.env.TEBAKGAMBAR_TTL_MS) || (60 * 1000);
...
function setSession(key, puzzle) {
    ...
    sessions.set(key, {
        index: puzzle.index,
        img: String(puzzle.img || ''),
        deskripsi: String(puzzle.deskripsi || ''),
...
function buildPuzzleCaption(session) {
    ...
    if (session.deskripsi) {
        parts.splice(3, 0, '', `Clue: ${session.deskripsi}`);
    }
    return parts.join('\n');
}
...
function tryAnswer(key, text) {
    ...
    if (session.expired) {
        clearSession(key);
        return { handled: true, result: 'expired', session };
    }
    ...
    if (guessNorm === session.answerNorm) {
        clearSession(key);
        return { handled: true, result: 'correct', session };
    }
